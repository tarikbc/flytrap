...
`deploy-to-flipper.py` uploads the fap to `/ext/apps/GPIO/` and copies the bundled
portals to `/ext/apps_data/flytrap/portals/`, verifying every file by on-device md5.
It uses the Flipper's serial CLI (`storage write_chunk` / `storage md5`).
